...
// POST /api/bookmarks — transaction: create bookmark + increment save_count atomically
router.post('/', auth, async (req, res) => {
    ...
    try {
        await runInTransaction(async (session) => {
            ...
            const doc = new SavedOpportunity({ user_id: req.user.user_id, opp_id });
            await doc.save(opts);
            await Opportunity.findByIdAndUpdate(opp_id, { $inc: { save_count: 1 } }, opts);
        });
...
// DELETE /api/bookmarks/:opp_id — transaction: remove bookmark + decrement save_count atomically
router.delete('/:opp_id', auth, async (req, res) => {
    try {
        await runInTransaction(async (session) => {
            const opts = session ? { session } : {};
            const deleted = await SavedOpportunity.findOneAndDelete(
                { user_id: req.user.user_id, opp_id: req.params.opp_id },
                opts
            );
            if (deleted) {
                await Opportunity.findByIdAndUpdate(req.params.opp_id, { $inc: { save_count: -1 } }, opts);
            }
        });
